...
整个打死;窗口快满的时候,这种死法不是靠改提示词能绕开的。在窗口达到
<!-- SAFE-WINDOW-TBD --> token 之前的足够余量,就该把 agent 的对话压缩或
清空,而不是等服务端替你收场。安全阈值本身还没有实测数字。

**实际能同时跑几条请求,不等于那个 flag 的数值。** serve 那一行的
`--max-num-seqs 20` 是调度器允许接纳的上限,不是 20 条请求会一起跑的
保证;当 KV 缓存池装不下那么多序列时,vLLM 会抢占到装得下的数量,而不是
让请求失败。在给定的 prompt 长度下,这套配置实际能撑住多少条并发请求,
还没有实测: <!-- CONCURRENCY-TBD -->。
...
池决定,数字是 <!-- CONCURRENCY-TBD -->。我往下调的只有利用率
