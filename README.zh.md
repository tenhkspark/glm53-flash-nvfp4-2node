...
路径;其他 id 一律返回 404。这条检查不是纸上谈兵:2026-09-17,一个客户端
还指着旧名字 `GLM-5.3-Flash-NVFP4`,每条请求都收到 404,直到把它改指向
`GLM-5.3-Flash-NVFP4-Wabi` 为止——把 served 名字对上就是全部修法,不存在
另外一张 id 到检查点的对照表要去查。`/v1/models` 会把它连同服务窗口一起返回:
...
服务的:用 pi,base URL 指向 `http://<head-node-ip>:<PORT>/v1`,模型指定
`GLM-5.3-Flash-NVFP4-Wabi`,拿回结构化的 `tool_calls`,thinking 也留在
自己的字段里,不会漏进回答。
...
**别让窗口被填满。** 下文"长上下文,实测"记录了 4 条提示,不管调
`--gpu-memory-utilization` 还是 `--max-num-batched-tokens` 都照样把引擎
整个打死;窗口快满的时候,这种死法不是靠改提示词能绕开的。把压缩当成客户端的常规动作,而不是出事之后才做的补救:在离窗口还有
很大余量的时候就定期把 agent 的对话折叠或清空,不要等服务端替你强制
收场。<!-- SAFE-WINDOW-TBD --> 标记的是安全阈值的具体数字应该填在的
位置,等实测出来再填;现在还没有测过,所以我不写一个自己没验证过的
数字——用留足余量的做法代替对某个数字的信任。

**保证范围。** 服务端的设置上限是 204,800 token,推荐配置是 EP off。用现有 EP-off
出货配置确认过的长文输入是 197,485 token;上限全域以及长文同时执行的稳定性,我
还没有验收完。READY 之后仍然需要额外内存。在 pi 中,对话历史会自动保存,
contextWindow 设为 163,840,默认设置下会在 147,456 附近开始自动压缩——这是客户端
一侧的推荐设置,不代表模型本身的上下文处理能力被改成了 147,456。`max-num-seqs=20`
是调度器上的上限,不保证 20 条请求同时常驻。罕见的引擎停止是可能发生的:停止时
请恢复对应引擎,再从已保存的会话与产出文件中续接未完成的工作,进行中的生成有可能
丢失。启动时对两个节点做 headroom 检查是一项可选的诊断,不是标准启动的必须条件,
也不是零落下的保证。

**实际能同时跑几条请求,不等于那个 flag 的数值。** serve 那一行的
`--max-num-seqs 20` 是调度器允许接纳的上限,不是 20 条请求会一起跑的
保证;当 KV 缓存池装不下那么多序列时,调度器不会让请求失败,也不会抢占
正在跑的序列,而是把多出来的放进等待队列。在这对节点上,每条 prompt
...
**建议在每次启动时检查主机的空闲内存。** 在两个节点上、发出第一个请求之前跑
`serve/check-headroom.sh` 是一项可选的诊断,不是标准启动的必须条件;但它是速度表
永远提醒不了你的一步运维动作,所以
单独给它一段。
...
变长会做的事。那次启动跑完了已验证的长度梯子,最长到
197,485 token,零失败;这不代表已覆盖 204,800 token 的完整窗口。
...
警告:**DGX OS 既不带 `earlyoom` 也不带 `systemd-oomd`。** 在我的节点上,
是我自己装的 OOM 守护进程把这件事变成了一次干净的进程终止。没有它,同样的
压力在这套硬件上就是一台挂死的机器。
...
更长的窗口不妨碍服务端以 `--max-num-seqs 20` 起来:204800 下能到 READY(用了 909 秒,随后
同一把标尺失败 0 条),在 307200 下才起不来。这只是启动时被接受的
