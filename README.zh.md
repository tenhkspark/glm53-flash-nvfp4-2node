...
哪份日志。有一行本可以让比较完全同条件,它仍在测量中;这一行标为
`pending`,而不是用相近的数字填上。
...
| 步骤 | 它隔离出什么 | 行 | 实测 |
|---|---|---|---|
| stock, no draft, RDMA | 快速 transport 上的起点 | pending | pending |
...
中间那两行现在共享全部条件——pair 1、NCCL/IB (RDMA)、同样的 64 条
日语散文提示、`temperature=0`、`max_tokens=512`、expert parallel
开启、thinking 关闭——所以 draft 可以干净地与其他一切分开:
...
35.09 / 27.15 = 1.29,所以在这个配置上,draft 在重新量化后的检查点
之上大约值 1.29x。这是本 README 中唯一一个只跨越单一条件变化取得的
比值;其余任何两行之间的差异都不止一处,所以我不去相除。
...
- **transport。** route h + MTP K=2、pair 1、64 条提示:sockets 下
  ...
  差距可以复现。
- **重新量化。** 仍然没有同条件的对照。重新量化自己的那一行现在
  已经在 pair 1 上经 RDMA 测得(27.15 tok/s,无 draft),但它作为
  分母所需的 stock 行还没有:我手上的 stock 参照是在 pair 2 上、经
  sockets、用标尺的前 8 条提示、以 `--max-model-len 131072` 测得的
  (10.75 tok/s),所以用它去除会一次混入三处条件变化。剩下的那一行
  pending 正是为此而测;在它落地之前,我不为重新量化印出任何倍数。
...
调用(tool 列是结构性的零)、指令遵循、超过 16384 token 服务窗口的
长上下文、安全行为,也没有测过开启 thinking 的情况——而这正是这个
...
固定 `--max-model-len 16384`、`--gpu-memory-utilization 0.85`、
FP8 KV 缓存;而实测各行跑的是变体——2026-09-13 的 stock 基线用
...
即上表里的那一行)。剩下的唯一一行 `pending` 是 stock 检查点、无
draft,在 pair 1 上经 RDMA、用同一把 64 条提示的标尺测——那是给重新
量化算加速倍数时所需要的分母。它正在测量中。

<!-- pending: stock-rdma-nodraft -->

这张表里故意没有 "vs stock" 列:我手上仅有的 stock 无 draft 行
取自不同的 pair、不同的 transport、8 条而非 64 条提示,所以任何
与它们相比的比值都会把三处条件变化折进一个数字里。
...
提示)和 RDMA 下 109.03 tok/s(pair 2,64 条提示);route h 目前
还没有 C=32 的行。
...
- 投机深度。K=1(34.28)与 K=2(35.09)在这个实验的轮间离散下分
  不开,而 K=3 与 K=4 更慢。更好的 draft 会改变那个最优点的位置。

**质量,尚未测量**

- 英语和其他语言。这里每一项探针都是日语。
- 代码正确性。代码提示出现在速度表里,在质量表里一处也没有。
- 多轮对话、工具调用、指令遵循、超过 16384 token 服务窗口的长
  上下文、安全行为,以及开启 thinking 的模型——而这正是这个家族
  通常的用法。
...
先补上剩下的那一行 pending、把按类型的其余几组跑完,然后是两种 K
