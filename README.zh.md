...
307200 token,但上面每一项探针都只有 2k 左右——
...
固定 `--max-model-len 307200`、`--max-num-seqs 2`、
...
**为什么出货的窗口是 307200,而表里不是。** 16384 是测量用的值——
...
35.07 tok/s,对比 16384 / 20 / 0.86 的 35.09——窗口拉长 18.75 倍,
在约 2% 的轮间离散里看不出差别。长窗口的代价是并发而不是速度——
在 307200 下引擎用 20 个槽位起不来,所以随附脚本只要 2 个;而 0.89
的利用率在这对节点上有过被拒绝启动的记录。窗口装得下,原因之一正是
重新量化:同样在 0.88 下,官方检查点的上限是 156672 token(vLLM 在
拒绝启动时会把这个上限打印出来),而 route h 在 307200 下能起来。
...
- 多轮对话、工具调用、指令遵循、长上下文——出货的窗口是 307200
