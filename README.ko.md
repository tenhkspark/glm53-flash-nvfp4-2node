...
동일 조건 비교를 완성할 한 개 행은 여전히 측정 중입니다; 대략적인
수치로 채워 넣는 대신 `pending`으로 표시해 두었습니다.
...
대한 네 개의 행이 필요합니다. 그중 세 개는 현재 존재하며, 네 번째는 측정 중입니다:
...
가운데 두 행은 이제 모든 조건 — pair 1, NCCL/IB (RDMA), 동일한 64개 일본어
산문 프롬프트, `temperature=0`, `max_tokens=512`, 전문가 병렬 켬, 사고 끔 — 을
공유하므로, 드래프트의 효과는 다른 모든 요소로부터 깔끔하게 분리됩니다:
...
35.09 / 27.15 = 1.29이므로, 이 구성에서 드래프트는 재양자화된
체크포인트 대비 약 1.29배의 가치가 있습니다. 이것은 이 README에서
단 하나의 조건 변경에 걸쳐 취해진 유일한 비율입니다. 다른 모든 행의 쌍은
둘 이상의 항목에서 차이가 나므로, 저는 그것들을 나누지 않습니다.
...
- **재양자화.** 여전히 동일 조건의 쌍이 없습니다. 재양자화 자체의 행은
  현재 pair 1에서 RDMA 상으로 측정되었으나 (27.15 tok/s, 드래프트 없음),
  분모로 필요한 순정(stock) 행은 그렇지 않습니다. 제가 보유한 순정 기준값은
  pair 2에서 sockets 상으로, `--max-model-len 131072`에서 ruler의
  첫 8개 프롬프트에 걸쳐 측정된 것(10.75 tok/s)이므로, 이것으로 나누면
  세 가지 조건 변경이 한꺼번에 섞이게 됩니다. 그것이 남아 있는 보류 행이
  존재하는 이유이며, 해당 값이 나올 때까지 저는 재양자화에 대한 배수를
  출력하지 않습니다.
...
16384-token 서빙 윈도우를 넘는 긴 문맥에 대해서도, 안전성 동작에 대해서도, 혹은 사고가 활성화된
...
`--max-model-len 16384`, `--gpu-memory-utilization 0.85`, FP8 KV 캐시로 고정하며, 측정된 행들은
변형들을 실행했습니다 — 2026-09-13 순정 기준선은 `--max-model-len 131072`를 사용했고,
...
분해의 route-h no-draft 단계는 2026-09-16에 도달했습니다
(27.15 tok/s, 위 행). 유일하게 남은 하나의 `pending` 행은 draft가 없는
stock 체크포인트이며, RDMA 기반 pair 1에서, 동일한 64개 프롬프트
ruler 상의 결과입니다 — 이는 requant에 대한 가속 계수에 필요한 분모입니다. 현재
측정 중입니다.
 
<!-- pending: stock-rdma-nodraft -->
 
이 표에는 의도적으로 "vs stock" 열이 없습니다: 제가 가진 유일한 stock
no-draft 행들은 다른 pair, 다른
전송 방식 및 64개가 아닌 8개의 프롬프트에서 측정되었으므로, 이에 대한 어떤 비율도
세 가지 조건 변경 사항을 하나의 숫자로 합치게 됩니다.
...
109.03 tok/s(pair 2, 64 prompts)였습니다; 경로 h에 대한 C=32 행은 아직 없습니다.
...
- 추측 깊이(Speculation depth). K=1 (34.28) 및 K=2 (35.09)는 본 실험의
  실행 간 편차로 구분되지 않으며, K=3 및 K=4는 더 느립니다. 더 나은
...
- 코드 정확성. 코드 프롬프트는 속도 표에 나타나며 품질 표에는 전혀
  나타나지 않습니다.
- 다중 턴 대화, 도구 호출, 지시 수행, 16384-토큰 서빙 윈도우를 넘어선 긴
  컨텍스트, 안전 동작, 그리고 사고 모드가 활성화된 모델(이 제품군이 통상적으로
  사용되는 방식)입니다.
...
남은 보류 행, 나머지 유형별 세트, 그리고 두 가지 K 순서 모두에서의 완전한
