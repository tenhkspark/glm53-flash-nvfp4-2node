...
**여기서 보증하는 범위를 밝혀 둡니다.** 설정상 상한은 204,800
토큰입니다. 기존 EP-off 출하 구성에서 확인한 장문 입력은 197,485
토큰이며, 상한 전 구간과 장문 동시 실행의 안정성은 아직 검수하지
못했습니다. READY 이후에도 추가 메모리가 필요합니다. 부팅 시 두 노드를
확인하는 것은 필수이지만, 그 통과가 무손실을 보장하는 것은 아닙니다.
권장 운용은 단일 실행입니다. 유한한 부팅 재시도와 단일 실행 설정을
결합한 다음 버전은 현재 검수 중입니다.
...
**부팅할 때마다 호스트의 남은 메모리를 확인하세요.** 두 노드 모두에서,
첫 요청을 보내기 전에 `serve/check-headroom.sh` 를 돌립니다. 속도 표로는
결코 알 수 없는 운영 단계라서 따로 한 단락을 씁니다.
