...
끝내주기를 기다리지 말고, <!-- SAFE-WINDOW-TBD --> 토큰에 이르기 훨씬
전에 에이전트의 대화를 압축하거나 지우십시오. 안전한 임계값 자체는
아직 실측하지 않았습니다.
...
못하면, vLLM 은 요청을 실패시키는 대신 담을 수 있는 만큼으로
선점(preempt)합니다. 주어진 프롬프트 길이에서 이 구성이 실제로 몇 개의
동시 요청을 버티는지는 아직 실측하지 않았습니다: <!-- CONCURRENCY-TBD -->.
...
그 수는 <!-- CONCURRENCY-TBD --> 입니다. 내린 플래그는 활용률
