...
일본어 산문이 눈에 띄게 끊겨서 출력되는 대신 화면에서 제가 읽는
속도와 거의 비슷하게 출력된다는 뜻입니다.

이 저장소는 해당 수치를 달성한 레시피입니다: 두 노드에 걸친 Ray 텐서
병렬화 (TP=2), 단일 스트림 디코드 속도를 복원하기 위해 제가 사용하는
가중치 전용 재양자화 (루트 h), RDMA를 지원하는 파생 이미지, MTP
드래프트를 로드할 수 있게 해 주는 오버레이를 포함합니다. 이 작업은
저 자신의 구현이며, 저의 자체 하드웨어에서 제가 직접 측정한 결과입니다.
다만 함께 배포되는 파일 중 하나는 제 것이 아닙니다 —
`overlays/flashinfer_mla_sparse_sm120.py`는 서빙 이미지 자체의 vLLM
소스 파일로, Apache-2.0 라이선스와 상류 저작권 헤더를 그대로 유지한
채 no-rope MLA를 지원하도록 여기서 수정한 것입니다. 나머지 오버레이는
빌드 시점에 같은 이미지 자체의 소스를 다시 쓰는, 제가 작성한 패처입니다.
어디까지가 상류이고 어디부터가 제 수정인지는 [NOTICE](NOTICE)가 파일
단위로 선을 긋습니다 — *미완성을 즐기며*.
...
몇 가지 개선 아이디어가 여전히 저의 목록에 남아 있지만, 모델이 제가
...
요청에 실어야 합니다. 다른 id는 404로 돌아옵니다. 이 확인은 탁상공론이
아닙니다: 2026-09-17, 이전 이름 `GLM-5.3-Flash-NVFP4` 를 그대로 가리키던
클라이언트가 `GLM-5.3-Flash-NVFP4-Wabi` 로 다시 가리킬 때까지 모든
요청에서 404를 받았습니다 — served 이름을 맞추는 것이 고치는 방법의
전부이며, 따로 찾아야 할 id-체크포인트 대응표는 없습니다. `/v1/models`
는 이 id를 서빙 창과 함께 돌려줍니다:
...
그렇게 설정한 코딩 에이전트에서 이 서버를 사용합니다: pi를 base URL
`http://<head-node-ip>:<PORT>/v1`, 모델 `GLM-5.3-Flash-NVFP4-Wabi` 로
향하게 하면, 구조화된 `tool_calls` 를 돌려받고 thinking은 답변으로
새지 않고 자기 필드에 남습니다.
...
**창을 다 채우게 두지 마십시오.** 아래 "긴 문맥, 실측"에는
`--gpu-memory-utilization` 이나 `--max-num-batched-tokens` 를 바꿔도
엔진째로 죽인 프롬프트 네 건이 실려 있습니다. 창이 거의 다 찼을 때는
이 죽는 방식을 프롬프트를 다듬어서 피할 수 없습니다. 서버가 대신
압축은 문제가 생긴 뒤에 하는 대응이 아니라 클라이언트의 기본 동작으로
삼으십시오 — 서버가 대신 끝내주기를 기다리지 말고, 창에 여유가 많이
남아 있는 시점에 정기적으로 에이전트의 대화를 접거나 지우는 운영으로
바꾸십시오. <!-- SAFE-WINDOW-TBD --> 는 실측이 끝나면 안전한 임계값
숫자를 채워 넣을 자리로 남겨 둔 표시입니다. 아직 실측하지 않았으므로
확인하지 않은 숫자는 쓰지 않습니다 — 숫자를 믿는 대신 여유를 두고
운영하십시오.

**보증 범위의 현재 상태.** 서버의 설정상 상한은 204,800 토큰이며,
권장 구성은 EP off입니다. 기존 EP-off 출하 구성에서 확인한 장문 입력은
197,485 토큰이며, 이는 설정상 상한 204,800과는 별개의 실측값이고, 상한
전 구간과 장문 동시 실행의 안정성은 아직 검수하지 못했습니다. READY
이후에도 추가 메모리가 필요합니다. pi에서는 이력을 자동 저장하며,
contextWindow를 163,840으로 두고 기본 설정이라면 147,456 부근에서 자동
컴팩션을 시작합니다. 147,456은 pi 쪽 클라이언트 설정이며, 모델의
컨텍스트 처리 능력을 147,456으로 변경했다는 의미가 아닙니다.
`max-num-seqs=20`은 스케줄러상의 상한이며 20개의 동시 상주를 보장하지
않습니다. 드문 엔진 정지가 발생할 수 있습니다. 정지 시에는 엔진을 복구하고,
저장된 세션과 산출 파일에서 미완료 작업을 재개하십시오. 진행 중이던
생성은 유실될 수 있습니다. headroom 확인은 임의의 진단이며, 표준 부팅의
필수 조건이 아닙니다. 부팅 시 두 노드를 확인하는 것은, 통과하면 안심할
근거가 되지만 무손실을 보장하는 것은 아닙니다.

**실제로 동시에 몇 개가 도는지는 플래그 값 그 자체가 아닙니다.** serve
줄의 `--max-num-seqs 20` 은 스케줄러가 받아들여도 되는 상한이지, 20개가
동시에 돈다는 보장이 아닙니다. KV 캐시 풀이 그만큼의 시퀀스를 담지
못하면, 요청을 실패시키거나 이미 돌고 있는 요청을 밀어내는 대신,
스케줄러는 나머지를 대기 큐에 넣습니다. 이 페어에서 프롬프트 한 개당
약 25,000 토큰짜리 요청 20개를 한 번에 보냈을 때(창 204,800 토큰,
EP on, KV 풀 382,740 토큰), KV 풀 용량의 128%에 달해도 대기 큐가
초과분을 깔끔하게 흡수했습니다: 스케줄러가 동시에 돌린 최대치는
6개, 나머지 14개는 대기 큐에서 기다렸고, 20개 전부 실패 0건·선점
0건으로 끝났습니다. 이 C=20 실측의 엔진 로그, 결과 JSON, head/worker 저수위 TSV는 `results/logs/route-h-seqs-c20-p1-engine.log`, `results/logs/route-h-seqs-c20-p1-result.json`, `results/logs/route-h-seqs-c20-p1-lowwater-head.tsv`, `results/logs/route-h-seqs-c20-p1-lowwater-worker.tsv`에 기록되어 있습니다. 즉 `--max-num-seqs 20` 은 서버가 받아들이는
상한일 뿐 동시에 도는 개수가 아니며, 이 프롬프트 길이에서 이 구성이
실제로 동시에 디코딩하는 개수는 **6개**입니다. 플래그를 낮출 필요는
없습니다.

**드문 낙하에 대비해, 부팅할 때마다 호스트의 남은 메모리를 확인하는
것을 권장합니다.** `serve/check-headroom.sh`를 두 노드 모두에서, 첫
요청을 보내기 전에 돌리는 임의의 진단입니다. 표준 부팅의 필수 조건이
아니라 운영 판단에 따른 선택입니다. 속도 표로는 결코 알 수 없는 운영
단계라서 따로 한 단락을 씁니다.

GB10 은 통합 메모리입니다. GPU 가 호스트 RAM 에서 할당하고, 그 예약 중
약 100 GB 는 어떤 표준 커널 카운터에도 나타나지 않습니다 — 이 칩에서
`nvidia-smi` 의 FB Memory Usage 는 N/A 를 돌려줍니다 — 그래서
`MemAvailable` 이 가진 유일한 정직한 계기입니다. vLLM 은 프로파일하던
순간에 마침 비어 있던 양으로 예산을 정하므로, **여유를 정하는 것은 플래그가
아니라 그 번의 부팅**입니다. 고치지 않은 같은 스크립트가 같은 노드에서 세 번의
부팅에 각각 5470, 9728, 10270 MiB 의 여유로 READY 에 도달했습니다. 제가 볼 수
있는 범위에서 아무 차이도 없는 부팅들 사이에 4.8 GiB 의 폭이 있습니다.

그리고 READY **이후에** 약 4.7 GiB 가 더 쓰입니다. 프롬프트를 21k → 32k →
64k → 128k → 197,485 토큰으로 올리면서 두 노드의 `MemAvailable` 을 1 초마다
받으면, 가장 좋았던 부팅은 10270 MiB(8.24%)에서 5579 MiB(4.48%)까지
떨어진 뒤 거기서 멈췄습니다. 누수가 아니라 한 번뿐인 최고 수위입니다 —
같은 길이를 반복해도 더 들지 않습니다 — 그러나 **더 큰 prefill 모양이 처음
올 때 청구됩니다**. 코딩 에이전트가 문맥을 늘려가며 하는 일이 바로 그것입니다.
그 부팅은 선언한 창 전체를 포함해 사다리를 끝까지 올랐고, 실패는 0 이었습니다.

5470 MiB 로 뜬 부팅은 그러지 못했습니다. 여섯 시간을 돌다가 20,915 토큰짜리
에이전트 프롬프트 도중에 죽었고, 드라이버가 먼저 `NV_ERR_NO_MEMORY` 를
남겼습니다. 자기 예열에 쓸 자리가 처음부터 없었던 것입니다.

그래서 바닥은 "예열에 드는 양 + 당신을 거둬가는 것의 선"이고, 확인은 부팅
시점에 합니다. 바닥 아래로 뜬 노드는 손보는 게 아니라 재시작해야 합니다.
`--gpu-memory-utilization` 을 낮추는 것은 보이는 만큼의 지렛대가 아닙니다 —
204800 에서 제한이 되는 rank 는 KV 를 약 3.5 GiB 밖에 못 받고 utilization 의
0.01 이 1.2 GiB 이므로, 두 칸만 내리면 엔진은 자기가 내건 창을 더 이상 열지
못합니다. 재현하려는 분께 드리는 경고 하나: **DGX OS 는 `earlyoom` 도
`systemd-oomd` 도 싣고 있지 않습니다.** 제 노드에서는 제가 직접 넣은 OOM
데몬이 이 일을 깔끔한 프로세스 종료로 바꿔 주었습니다. 그것이 없으면 이
하드웨어에서 같은 압력은 멈춰 버린 노드가 됩니다.
...
그대로 고정되어 있었습니다. 2026-09-17에 출하하는 윈도우에서 다시 측정한
결과는 — 이 표의 다른 모든 행과 마찬가지로 expert parallel을 켠 상태로 —
...
`--enable-expert-parallel` 을 넘기지 않지만, 이 표의 모든 행은 그것을 켠
채로 측정한 것입니다. 공개된 스크립트를 출하 상태 그대로 돌리면, 같은
64개 프롬프트 ruler가 갓 부팅한 페어에서 **37.33 tok/s** 를 냈습니다 —
TPOT 중앙값 26.4 ms, TTFT 중앙값 0.302 s, 64건 중 실패 0건, 수용률
0.6168 — 그리고 재측정에서 36.95였습니다. 같은 스크립트에 EP 두 플래그를
되돌리면 35.05(TPOT 27.9 ms, 수용률 0.6223)로 떨어지고, 이는 34.78 행을
0.8% 이내로 재현합니다. 그러니 차이는 expert parallel 하나뿐이고 다른
것은 없습니다. EP는 여기서 호스트 메모리도 더 먹습니다: READY 이후의
최고 수위를 4691 MiB에서 6014 MiB로 올리고, 사다리의 최저 수위를
4.48%에서 3.38%로 낮췄습니다. 이 페어에서는 더 느리고 더 배가 고프므로
스크립트는 그것을 출하하지 않습니다 — 위의 표도 더 빠른 숫자를 빌려오지
않고 자기가 측정한 조건을 그대로 둡니다.

긴 윈도우라고 해서 `--max-num-seqs 20` 으로 서버가 못 뜨는 것은
아닙니다: 204800에서는 READY에 도달합니다(909초, 그다음 같은
ruler에서 실패 0건)만, 307200에서는 뜨지 않았습니다. 이는 부팅 시
받아들여졌다는 사실일 뿐, 받아들여진 20개 슬롯 중 실제로 몇 개를
동시에 돌릴 수 있는지의 실측이 아닙니다 — 그것은 KV 풀이 정하며,
그 수는 **6개**입니다 — 20개를 한 번에 보내면 14개가 대기 큐에서
기다리고 6개가 동시에 돌며, KV 풀 용량의 128%에서도 실패 0건·선점
0건이었습니다. 이 C=20 실측의 엔진 로그, 결과 JSON, head/worker 저수위 TSV는 `results/logs/route-h-seqs-c20-p1-engine.log`, `results/logs/route-h-seqs-c20-p1-result.json`, `results/logs/route-h-seqs-c20-p1-lowwater-head.tsv`, `results/logs/route-h-seqs-c20-p1-lowwater-worker.tsv`에 기록되어 있습니다. `--max-num-seqs 20` 은 그대로 출하합니다. 이 값은
스케줄러가 받아들이는 상한을 정할 뿐 동시 디코딩 개수를 정하지
않으며, 낮춰도 이 6개는 바뀌지 않습니다. 내린 플래그는 활용률
...
1. **부팅 시 호스트 여유 부족.** 같은 스크립트로 부팅해도 READY 시점의
   여유가 5,470~10,270 MiB 사이에서 흔들립니다. 여유가 적게 뜬 부팅은
   나중에 긴 프롬프트 한 건만으로도 죽습니다.
2. **Ray의 OOM 감시.** 두 노드 모두에서
   `RAY_memory_usage_threshold=0.99`를 export하지 않으면, Ray가 기본값
   0.95에서 vLLM의 TP0 worker를 kill합니다(이미 대응했지만 설정이
   필수입니다).
3. **장문에서의 수요 초과.** 약 259,000 토큰급 프롬프트는 현재 창의
   바깥이며, utilization을 낮추거나 chunk를 낮춰도 과거에는 죽었습니다.
   창의 상단은 아직 검수하지 않았습니다.
4. **메모리가 원인이 아닌 것들.** `--max-num-batched-tokens 2048`은
   여유가 충분해도 KDA 커널의 illegal memory access를 만납니다.
   `nccl-ll-symm-ar` 구성은 빈 응답을 돌려주었습니다. 둘 다 출하에서
   제외했습니다.

크래시가 수정되었다고는 쓰지 않습니다. 부팅 시 검사는 선별이지
수정이 아닙니다.

**작동하지 않는다고 제가 이미 알고 있는 것들.** 조밀 선형 레이어(dense linears)에 대한 FP8,
산문에서의 3 이상의 K, 그리고 eager 모드 강제입니다. 그
