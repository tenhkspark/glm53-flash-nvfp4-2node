...
コンパクト化は失敗が起きてからの対処ではなく、クライアント側の
標準動作にしてください——サーバに強制終了させられる前に、窓を
使い切るよりずっと手前のタイミングで、エージェントの会話を定常的に
畳むかクリアする運用にします。<!-- SAFE-WINDOW-TBD --> は、実測でき
た時点で安全な閾値の数値を入れる場所です。まだ実測していないため、
確かめていない数値は書きません——数値に頼るのではなく、余裕を
持たせて運用してください。

**保証範囲の現状。** サーバの設定上限は204,800トークン、推奨構成は
EP offです。既存のEP-off出荷構成で確認した長文入力は197,485トークン
で、これは設定上限204,800とは別の実績値であり、上限全域および長文
同時実行の安定性はまだ検収できていません。READY後にも追加メモリが
必要です。piでは履歴を自動保存し、contextWindowを163,840として、
既定設定なら147,456付近で自動コンパクションを開始します。147,456は
pi側のクライアント設定であり、モデルのコンテキスト処理能力を
147,456に変更したという意味ではありません。`max-num-seqs=20`は
スケジューラ上の上限で、20本の同時常駐を保証しません。稀な
エンジン停止は起こり得ます。停止時はエンジンを復旧し、保存
セッションと成果ファイルから未完了作業を再開してください。進行中の
生成は失われることがあります。headroomチェックは任意の診断であり、
標準起動の必須条件ではありません。起動時の両ノード確認は、通れば
安心材料になりますが、無落下を保証するものではありません。

**実際に何本同時に走るかはフラグの値そのものではありません。** serve行の
`--max-num-seqs 20` はスケジューラが受け入れてよい上限であって、20本が
同時に走る保証ではありません。KVキャッシュプールがその本数を保持できない
ときは、リクエストを失敗させたり実行中のものを追い出したりする代わりに、
スケジューラは残りを待ち行列に入れます。このペアで1本あたり約25,000
トークンのプロンプトを20本同時に送ったところ（窓204,800トークン、EP on、
KVプール382,740トークン）、プール容量の128%に達してもきれいに待ち行列で
吸収されました: スケジューラが同時に走らせたのは最大6本、残り14本は
待ち行列で待ち、20本すべてが失敗0件・プリエンプト0件で完了しています。 このC=20実測のエンジンログ・結果JSON・head/worker側の低水位TSVは `results/logs/route-h-seqs-c20-p1-engine.log`、`results/logs/route-h-seqs-c20-p1-result.json`、`results/logs/route-h-seqs-c20-p1-lowwater-head.tsv`、`results/logs/route-h-seqs-c20-p1-lowwater-worker.tsv` に記録されています。
つまり `--max-num-seqs 20` はサーバが受け付ける上限であって同時に走る
本数ではなく、このプロンプト長でこの構成が実際に同時デコードする本数は
**6本**です。フラグを下げる必要はありません。

**稀な落下に備え、起動のたびにホストの空きメモリを確認することを
お勧めします。** `serve/check-headroom.sh` を両ノードで、最初の
リクエストを送る前に走らせる、任意の診断です。標準起動の必須条件
ではなく、運用判断による選択です。速度表からは決して分からない
運用手順なので、独立した段落を割きます。
...
この窓と1本あたり約25,000トークンのプロンプトでは、その本数は
**6本**です——20本を同時に送ると14本が待ち行列に入り、6本が同時に
走り、KVプール容量の128%でも失敗0件・プリエンプト0件でした。 このC=20実測のエンジンログ・結果JSON・head/worker側の低水位TSVは `results/logs/route-h-seqs-c20-p1-engine.log`、`results/logs/route-h-seqs-c20-p1-result.json`、`results/logs/route-h-seqs-c20-p1-lowwater-head.tsv`、`results/logs/route-h-seqs-c20-p1-lowwater-worker.tsv` に記録されています。
`--max-num-seqs 20` はそのまま出荷します。この値はスケジューラが
受け付ける上限を決めるだけで、同時デコード本数を決めるものではなく、
下げても6本は変わりません。下げた
