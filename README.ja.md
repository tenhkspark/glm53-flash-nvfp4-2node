...
派生イメージ、そしてMTPドラフトをロードさせるオーバーレイ。ここに
あるものはすべて私自身の実装と、私自身のハードウェアでの私自身の
実測値です——*不完全を楽しむ*。

以下の数値にはすべて条件(ペア、transport、プロンプト本数、K、
expert parallel、`max_tokens`)が付いており、速度表の各行は、その値
が出たログの名前を示します。比較を厳密に同条件にするための一行は
まだ測定中です。近い値で埋めず `pending` と記してあります。
...
四行が必要です。そのうち三行はいま手元にあり、四行目は測定中です。

| 段 | 何を切り分けるか | 行 | 実測 |
|---|---|---|---|
| stock, no draft, RDMA | 速いtransport上の出発点 | pending | pending |
...
中央の二行は、いまやすべての条件を共有しています——pair 1、
...
35.09 / 27.15 = 1.29なので、この構成ではドラフトは再量子化した
チェックポイントの上でおよそ1.29倍にあたります。これは、このREADMEで
条件変化を一つだけ挟んで取った唯一の比です。ほかの行の組はどれも二つ
以上が違うので、私は割りません。
...
- **再量子化。** 同条件の対はまだありません。再量子化自身の行は
  pair 1・RDMAで測れました(27.15 tok/s、ドラフト無し)が、分母として
  要るstockの行はまだです。手元のstock参照はpair 2・sockets・
  ルーラー先頭8プロンプト・`--max-model-len 131072` で取ったもの
  (10.75 tok/s)なので、それで割ると三つの条件変化が一度に混ざります。
  残るpending行はそのためにあります。着地するまで、再量子化の倍率は
  書きません。
...
16384トークンのサービング窓を超える長文脈、安全性の挙動、そして
...
2026-09-13のstockベースラインは `--max-model-len 131072`、
c1pairとroute hの行は `--gpu-memory-utilization 0.86`、
expert parallelは `ep` = `off` と記したstock + MTPの二行
(17.84と19.05 tok/s)でオフ、それ以外ではオンでした。各行の正確な
フラグは、`results/results.tsv` のその行の `source_log` が指す
`results/logs/` 以下のファイルにあります。
...
分解のroute h・ドラフト無しの脚は2026-09-16に着地しました
(27.15 tok/s、上の行)。残る `pending` 行は一つ、stockチェックポイントの
ドラフト無しをpair 1・RDMA・同じ64プロンプトのルーラーで測るもの
——再量子化の速度倍率が分母として必要とする行です。いま測定中です。

<!-- pending: stock-rdma-nodraft -->

この表に "vs stock" 列が無いのは意図的です。手元にあるstockの
ドラフト無し行は、別のペア・別のtransport・64本ではなく8本の
プロンプトで取ったもので、それに対する比はどれも三つの条件変化を
一つの数値へ畳み込んでしまうからです。
...
64プロンプト)でした。route hのC=32行はまだありません。
...
- 投機の深さ。K=1(34.28)とK=2(35.09)はこの実験の実行ごとのばらつきでは
  分離できず、K=3とK=4はより遅いです。よいドラフトがあれば、その最適点
  の位置は変わります。
...
- コードの正しさ。コードのプロンプトは速度の表には出てきますが、品質の表
  には出てきません。
- 複数ターンの会話、tool呼び出し、指示追従、16384トークンのサービング窓
  を超える長文脈、安全性の挙動、そしてthinkingを有効にしたモデル——この
  族が普段使われる形です。
...
**すでに効かないと分かっているもの。** dense linearへのFP8、expert
parallelのオフ、散文でのK=3以上、そしてeagerモードの強制です。その
数値と、測定したときの条件は上の失敗表にあります。どれかを試し直す前に
見る場所です。
...
残るpending行、種類別の残りの集合、そして両方向のK順での完全な
